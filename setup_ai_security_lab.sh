...
"
    ...
    # AI Red-Teaming Playground Labs is intentionally NOT in this list.
    # Its .NET Semantic Kernel embedding service hardcodes api.openai.com
    # at the SDK level and ignores any endpoint override. Writing OPENAI_*
    # there with a non-OpenAI base URL would mislead users into thinking
    # AI-RTP is wired. Students who want AI-RTP must use a real OpenAI or
    # Azure OpenAI key and configure that repo's .env manually.
    local repo_envs=(
...
$(echo -e "${YELLOW}IMPORTANT: Post-install steps${NC}")
  ...
  2. Configure the LLM router (one CLI call propagates to every supported lab):
       sudo lab-llm configure --model gpt-4o-mini --api-key sk-...
       sudo lab-llm doctor                              # verify endpoint

     This writes ${LAB_LLM_ENV}, ~/.pyrit/.env, each supported repo's .env,
     and ${LAB_ROOT}/promptfooconfig.yaml. To switch model or provider
     later, just re-run 'lab-llm configure'.

     NOTE: AI-Red-Teaming-Playground-Labs is NOT wired to the unified router.
     Its .NET Semantic Kernel embedding service hardcodes api.openai.com at
     the SDK level. To use that lab, edit its .env with a real OpenAI or
     Azure OpenAI key. See section 10.1 of AI_SECURITY_LAB_GUIDE.md.
